...
where songs_per_invoice AS (
select invoice.*, track.* from invoice 
...
select *,
case
    when (select (*) where invoice_id = songs_per_invoice.invoice_id)
from songs_per_invoice 
''')
```
...
select * from todo_data inner join 
''')
```
